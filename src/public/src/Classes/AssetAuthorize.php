...
class AssetAuthorize
{
  ...
  {
    $sql = "SELECT COUNT(*) FROM factory.asset_authorize
    WHERE user = ?";
  ...
  {
    $sql = "INSERT INTO factory.asset_authorize( `user`, `type`) VALUES(?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE factory.asset_authorize SET
    status = 0,
    updated = NOW()
    WHERE id = ?";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "SELECT COUNT(*) FROM factory.asset_authorize a WHERE a.status = 1";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    ...
    FROM factory.asset_authorize a
